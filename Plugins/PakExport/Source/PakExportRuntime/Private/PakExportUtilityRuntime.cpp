...
void UPakExportUtilityRuntime::GenerateJsonsForAssets(const TArray<FString>& InAssets, const FString& DestinationFile)
{
	...
	{
		const auto PakName = "/" + Asset.Replace(TEXT("Content"), *FString(UPakExportRuntimeStatic::PakExportName), ESearchCase::CaseSensitive)
	...
	for (const auto& SelectedAsset : InAssets)
	{
		...
		if (Asset->IsA(UMaterialInterface::StaticClass()))
		{
			if (!MaterialsPakRequested)
			{
				...
				{
					FMaterialBasePayload MaterialPakData;
...
FCameraPresetPayload UPakExportUtilityRuntime::GenerateCameraData(AActor* CameraActor, float FocusOffset/* = 0.f*/)

{
	FCameraPresetPayload CameraData;
